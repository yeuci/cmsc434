...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function statusDotClass(isoDate, soonDays) {
    const days = daysUntil(isoDate);
    if (days === null) return "dot-fresh";
    if (days < 0) return "dot-expired";
    if (days <= Math.max(0, soonDays ?? 2)) return "dot-soon";
    return "dot-fresh";
  }
  ...
  function render() {
    ...
    if (legend) {
      legend.innerHTML = `
        ...
        <span class="dot dot-soon"></span> Expires soon (≤ ${Math.max(
          0,
          settings.soonDays
        )}d)
  ...
  function openEditItemModal(itemId) {
    ...
    ingQtyInput.value = (Number.isFinite(+it.qty) && +it.qty > 0 ? +it.qty : 1).toString();
  ...
  addForm.addEventListener("submit", (e) => {
    ...
    if (itemModalCtx.mode === "edit" && itemModalCtx.targetId) {
      ...
      if (idx !== -1) {
        items[idx] = { ...items[idx], name, expiry, qty, updatedAt: Date.now() };
        saveItems(items);
      }
      itemModalCtx = { mode: "create", targetId: null };
      closeModal(addModal);
